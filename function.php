...
function register($data){
    ...
    //validasi akun
    $result = mysqli_query($db, "SELECT email FROM akunmahasiswa
            WHERE email = '$email'");
    ...
    //query insert data
    $query = "INSERT INTO akunmahasiswa VALUES 
    ...
    if($insert_result && mysqli_affected_rows($db)>0){
        $_SESSION['email'] = $email;
        $_SESSION['username'] = $username;
        $_SESSION['code'] = $code;
        $_SESSION["successMessage"] = "Berhasil Ditambahkan, Halaman mu akan diahlikan beberapa saat";
    } else {
        $_SESSION["errMessage"] = "Tidak Berhasil Ditambahkan. Mohon Coba lagi";
    }
...
function login($data){
    ...
    if (mysqli_num_rows($result) === 1){
        ...
        if (password_verify($password, $account['password'])){
            ...
            $_SESSION['location'] = "Admin/Homepage.php";

            //remember me (cookies)
            if (isset($_POST['remember'])){
                $_SESSION['path'] = "akunadmin";
        ...
        $result = mysqli_query($db, "SELECT * FROM akunmahasiswa WHERE email = '$email'");
        if (mysqli_num_rows($result) === 1){
            $account = mysqli_fetch_assoc($result);
            if($account['is_verif'] == 1){
                if (password_verify($password, $account['password'])){
                    //must be login first
                    $_SESSION["login"] = true;
                    $_SESSION['username'] = $account['username'];
                    $_SESSION['location'] = "Mahasiswa/Homepage.php";
        
                    //remember me ($_cookies)
                    if (isset($_POST['remember'])){
                        $_SESSION['path'] = "akunmahasiswa";
                        setcookie('hidden1', $account['id'], time()+108000);
                        setcookie('key', hash('sha256', $account['email'],time()+108000));
                    }
                    ...
                    // echo '<h3>Gagal Mahasiswaaa</h3>';
                    $_SESSION["errMessage"] = "Email atau password salah!";
                }
            } else {
                $_SESSION["errMessage"] = "Akun belum terverifikasi! Silahkan melakukan verifikasi di email Anda.";
            }
        } else {
            $_SESSION["errMessage"] = "Akun tidak terdaftar!";
        }
    // echo json_encode($data);
    }
}
